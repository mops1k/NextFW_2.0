...
class DB {
    ...
    public static function init($name = 'default',$array = [])
    {
        ...
        if(!is_array($array) OR count($array) == 0)
            throw new DBException("Неверная конфигурация подключения к базе данных!");
        ...
        if($error)
        {
            ...
            $errorText .= "\n";
            throw new DBException($errorText);
        }
    ...
    public static function setType($res)
    {
        self::setReturnResult($res);
    }
    ...
    {
        $params = [];
        if(count($vals) == 0) throw new DBException('Не указаны параметры для обновления записи(ей)');
        $valsLine = null;
        foreach ( $vals as $key => $value ) {
            $self = false;
            $valsArr = explode(" ",$value);
            foreach ($valsArr as $arr) {
                if($arr == "this") $self = true;
            }

            if($self)
            {
                $valsLine .= '`'.$key.'` = '.$key.' '.$valsArr[1].' :'.$key.'Vals, ';
                $params[":{$key}Vals"] = $valsArr[2];
            } else {
     ...
     * @param string $table Таблица
     * @param array $values Массив ключ => значение, аналогично столбец = значение
     *
     * @return bool
     */
    public static function insert($table,$values)
    {
        if(!is_array($values) || count($values) == 0) throw new DBException('Неверный формат значений для вставки');
        $valuesLine1 = "(";
        $valuesLine2 = "(";
        $params = [];
